...
import admin.bean.MoneyOut;
import admin.model.MoneyInDao;
import admin.model.MoneyOutDao;
 ...
 * @author gagan
 */
@WebServlet(name = "MoneyOutServlet", urlPatterns = {"/MoneyOutServlet"})
public class MoneyOutServlet extends HttpServlet{
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
       ...
       MoneyOutDao mid=new MoneyOutDao();
      ...
      //  min.setDid(Integer.parseInt(request.getParameter("did")));
        ...
        int status=mid.sendMoney(min);
        if(status>0)
        {
            response.sendRedirect("money_out_success.jsp");
        }
        else
        {
            response.sendRedirect("money_out.jsp");
        }
